...
class WLSM_Class {
		...
		if ( count( $filter_rows_limit ) ) {
			foreach ( $filter_rows_limit as $row ) {
				...
				$data[] = array(
					...
					<a class="text-danger wlsm-delete-class" data-nonce="' . esc_attr( wp_create_nonce( 'delete-class-' . $row->ID ) ) . '" data-class="' . esc_attr( $row->ID ) . '" href="#" data-message-title="' . esc_attr__( 'Please Confirm!', 'school-management' ) . '" data-message-content="' . esc_attr__( 'This will delete all data associated with this class.', 'school-management' ) . '" data-cancel="' . esc_attr__( 'Cancel', 'school-management' ) . '" data-submit="' . esc_attr__( 'Confirm', 'school-management' ) . '"><span class="dashicons dashicons-trash"></span></a>'
				);
			}
		}
	...
	public static function save() {
		...
		try {
			...
			if ( $class_exist ) {
				$errors['label'] = esc_html__( 'Class already exists.', 'school-management' );
			}
			// End validation.
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				if ( $class_id ) {
					...
					$message = esc_html__( 'Class updated successfully.', 'school-management' );
					$reset   = false;
				} else {
					$data['created_at'] = current_time( 'Y-m-d H:i:s' );

					$success = $wpdb->insert( WLSM_CLASSES, $data );
					$message = esc_html__( 'Class added successfully.', 'school-management' );
					$reset   = true;
				}
	...
	public static function delete() {
		...
		try {
			...
			$message = esc_html__( 'Class deleted successfully.', 'school-management' );
