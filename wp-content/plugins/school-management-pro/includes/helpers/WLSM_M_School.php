...
class WLSM_M_School {
	public static function get_page_url() {
		return admin_url( 'admin.php?page=' . WLSM_MENU_SCHOOLS );
	}

	public static function fetch_query() {
		$query = 'SELECT s.ID, s.label, s.phone, s.email, s.address, COUNT(DISTINCT cs.ID) as classes_count, COUNT(DISTINCT a.staff_id) as admins_count, s.is_active FROM ' . WLSM_SCHOOLS . ' as s 
		LEFT OUTER JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.school_id = s.ID 
		LEFT OUTER JOIN ' . WLSM_STAFF . ' as sf ON sf.school_id = s.ID AND sf.role = "admin" 
	...
	public static function fetch_query_count() {
		$query = 'SELECT COUNT(s.ID) FROM ' . WLSM_SCHOOLS . ' as s';
		return $query;
	}

	public static function get_school( $id ) {
		global $wpdb;
		$school = $wpdb->get_row( $wpdb->prepare( 'SELECT s.ID, s.enrollment_prefix, s.enrollment_base, s.enrollment_padding, s.admission_prefix, s.admission_base, s.admission_padding FROM ' . WLSM_SCHOOLS . ' as s WHERE s.ID = %d', $id ) );
		return $school;
	}
	...
	public static function fetch_school( $id ) {
		global $wpdb;
		$school = $wpdb->get_row( $wpdb->prepare( 'SELECT s.ID, s.label, s.phone, s.email, s.address, s.is_active, s.enrollment_prefix, s.enrollment_base, s.enrollment_padding, s.admission_prefix, s.admission_base, s.admission_padding FROM ' . WLSM_SCHOOLS . ' as s WHERE s.ID = %d', $id ) );
		return $school;
	}
	...
	public static function create_default_sections( $school_id ) {
		...
		foreach ( $classes as $class ) {
			...
			if ( count( $section_ids ) ) {
				...
				$section_data['created_at'] = current_time( 'Y-m-d H:i:s' );
			...
			if ( $default_section_id ) {
				$success = $wpdb->update(
					WLSM_CLASS_SCHOOL,
					array( 'default_section_id' => $default_section_id, 'updated_at' => current_time( 'Y-m-d H:i:s' ) ),
					array( 'ID' => $class->ID )
				);
