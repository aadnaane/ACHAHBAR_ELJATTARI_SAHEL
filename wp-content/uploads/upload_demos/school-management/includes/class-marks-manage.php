...
class Marks_Manage
{
	...
	public function mj_smgt_get_marks_pass_fail($exam_id,$class_id,$user_id)
	{
		global $wpdb;
		$table_name = $wpdb->prefix . "marks";
		return $retrieve_result = $wpdb->get_var( "SELECT marks FROM $table_name WHERE exam_id = $exam_id AND class_id=".$class_id." AND student_id = $user_id");
	}
	...
	public function mj_smgt_get_max_marks($exam_id)
	{
		...
		return $retrieve_result = $wpdb->get_var( "SELECT total_mark FROM $table_name WHERE exam_id = $exam_id");
	}
	...
	public function mj_smgt_get_pass_marks($exam_id)
	{
		global $wpdb;
		$table_name = $wpdb->prefix . "exam";
		return $retrieve_result = $wpdb->get_var( "SELECT passing_mark FROM $table_name WHERE exam_id = $exam_id");
	}
	...
	public function mj_smgt_get_exam_term($exam_id)
	{
		global $wpdb;
		$table_name = $wpdb->prefix . "exam";
		return $retrieve_result = $wpdb->get_var( "SELECT exam_term FROM $table_name WHERE exam_id = $exam_id");
	}
	...
	{
		$exam_id1=(int)$exam_id;
		$class_id1=(int)$class_id;
		$subject_id1=(int)$subject_id;
		$user_id1=(int)$user_id;

		global $wpdb;
		$tbl_grade = $wpdb->prefix . "grade";
		$tbl_marks = $wpdb->prefix . "marks";
		$retrieve_result = $wpdb->get_var( "SELECT g.grade_name,m.marks,m.mark_id FROM $tbl_grade g, $tbl_marks m where exam_id = $exam_id1 AND class_id=".$class_id1." AND subject_id = $subject_id1 AND student_id = $user_id1 AND m.marks between g.mark_upto and g.mark_from group by m.mark_id");
