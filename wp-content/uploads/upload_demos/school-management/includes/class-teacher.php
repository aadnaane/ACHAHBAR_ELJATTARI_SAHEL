...
class Smgt_Teacher
{
	public function mj_smgt_add_muli_class($classes,$name)
	{
		global $wpdb;
	...
	public function mj_smgt_add_muli_class_import($classes,$name)
	{
		global $wpdb;
		$table = $wpdb->prefix . 'smgt_teacher_class';
		$teacher = get_user_by('login',$name);
		...
		if(!empty($classes) && !empty($teacher))
		{
			foreach($classes as $class)
			{
				$class_id=mj_smgt_get_class_id_by_name($class);
				$success = $wpdb->insert($table,array('teacher_id'=>$teacher->ID,
											'class_id'=>$class_id,
											'created_by'=>$created_by,
											'created_date'=>$created_date));
	...
	public function mj_smgt_get_teacher_class($teacher_id)
	{
		global $wpdb;
		$table = $wpdb->prefix . 'smgt_teacher_class';
	...
	public function mj_smgt_get_class_teacher($class_id)
	{
		global $wpdb;
		$table = $wpdb->prefix . 'smgt_teacher_class';
	...
	public function mj_smgt_get_singal_class_teacher($class_id)
	{
		global $wpdb;
		$table = $wpdb->prefix . 'smgt_teacher_class';
	...
	public function mj_smgt_update_multi_class($classes,$teacher_id)
	{
		global $wpdb;
		$table = $wpdb->prefix . 'smgt_teacher_class';
		$created_by = get_current_user_id();
		...
		$old_class = $this->mj_smgt_get_teacher_class($teacher_id);
	...
	public function mj_smgt_get_class_by_teacher($teacher_id){
		global $wpdb;
		$table = $wpdb->prefix . 'smgt_teacher_class';
	...
	public function mj_smgt_get_class_by_teacher_notification($teacher_id){
		global $wpdb;
		$classes = array();
		$table = $wpdb->prefix . 'smgt_teacher_class';
		$data = $wpdb->get_results("SELECT class_id FROM {$table} WHERE teacher_id = {$teacher_id}",ARRAY_A);
	...
	function mj_smgt_in_array_r($needle, $haystack, $strict = false) {
		foreach ($haystack as $item) {
			if (($strict ? $item === $needle : $item == $needle) || ( is_array($item) && $this->mj_smgt_in_array_r($needle, $item, $strict))) {
				return true;
			}
		}
	...
	function mj_smgt_get_teacher_by_class($class_id = null)
	{
		global $wpdb;
		$table = $wpdb->prefix . 'smgt_teacher_class';
		if($class_id != null)
