...
function mj_smgt_submit_setupform($data)
{
	...
	elseif($result == '2')
	{
		$message = esc_attr__('This purchase key is already registered with the different domain. If have any issue please contact us at sales@dasinfomedia.com','school-mgt');
			$_SESSION['cmgt_verify'] = '2';
	}
	elseif($result == '3')
	{
		$message = esc_attr__('There seems to be some problem please try after sometime or contact us on sales@dasinfomedia.com','school-mgt');
...
function mj_smgt_datatable_multi_language()
{
	$datatable_attr=array("sEmptyTable"=> esc_attr__("No data available in table","school-mgt"),
		...
		"sInfoPostFix"=> "",
		"sInfoThousands"=>",",
		"sLengthMenu"=> esc_attr__("Show _MENU_ entries","school-mgt"),
		"sLoadingRecords"=> esc_attr__("Loading...","school-mgt"),
		"sProcessing"=> esc_attr__("Processing...","school-mgt"),
		"sSearch"=>esc_attr__("Search:","school-mgt"),
...
function mj_smgt_change_menutitle($key)
{
	...
	$menu_titlearray=array('virtual_classroom'=> esc_attr__('Virtual Classroom','school-mgt'),'teacher'=> esc_attr__('Teacher','school-mgt'),'student'=> esc_attr__('Student','school-mgt'),'child'=> esc_attr__('Child','school-mgt'),'parent'=> esc_attr__('Parent','school-mgt'),'subject'=> esc_attr__('Subject','school-mgt'),'class'=> esc_attr__('Class','school-mgt'),'schedule'=> esc_attr__('Class Routine','school-mgt'),'attendance'=> esc_attr__('Attendance','school-mgt'),'exam'=> esc_attr__('Exam','school-mgt'),'manage_marks'=> esc_attr__('Manage Marks','school-mgt'),'feepayment'=> esc_attr__('Fee Payment','school-mgt'),'payment'=> esc_attr__('Payment','school-mgt'),'transport'=> esc_attr__('Transport','school-mgt'),'hostel'=> esc_attr__('Hostel','school-mgt'),'notice'=> esc_attr__('Notice Board','school-mgt'),'message'=> esc_attr__('Message','school-mgt'),'holiday'=> esc_attr__('Holiday','school-mgt'),'library'=> esc_attr__('Library','school-mgt'),'account'=> esc_attr__('Account','school-mgt'),'report'=> esc_attr__('Report','school-mgt'),'homework'=> esc_attr__('Homework','school-mgt'));
...
function mj_smgt_get_student_groupby_class()
{
	...
	{		
		...
		if($class_id != '')
		{
			...
			$students[$classname][$student_obj->ID]=mj_smgt_get_user_name_byid($student_obj->ID)." ( ".get_user_meta($student_obj->ID, 'roll_id',true)." )";
		}
	}
	}
	return $students;
...
<br /><a href="<?php echo wp_logout_url(); ?>"><?php esc_attr_e('Logout','school-mgt');?></a> 
<?php 
	 }
	 else 
	 {
	   ...
	   else
	   {
		 ...
		 foreach($rResult as $aRow)
		 {
			...
			if(!empty($attchment))
			{
				...
				{					
					$view_attchment.='<a target="blank" href="'.content_url().'/uploads/school_assets/'.$attchment_data.'" class="btn btn-default"><i class="fa fa-download"></i>'. esc_attr__('View Attachment','school-mgt').'</a></br>';
				}
				$row[4] =$view_attchment;
			}
			else
			{
				 $row[4] = esc_attr__('No Attachment','school-mgt');
			}	
			$row[5] = mj_smgt_convert_date_time($aRow['created_date']);
			$row[6] = '<a href="?page=smgt_message&tab=view_all_message_reply&action=delete_users_reply_message&users_reply_message_id='.$aRow['id'].'" class="btn btn-danger" onclick="return confirm('. esc_attr__("'Are you sure you want to delete this message?'","'school-mgt'").')">'. esc_attr__('Delete','school-mgt').'</a>';
			$output['aaData'][] = $row;
		}
	 ...
	 if ( isset( $_REQUEST['iDisplayStart'] ) && $_REQUEST['iDisplayLength'] != '-1' )
	 {
	   ...
	   else
	   {
		...
		{			 
			...
			{					
				$row[4] = esc_attr_e('All','school-mgt');
			}
			...
			if(!empty($attchment))
			{
				...
				foreach($attchment_array as $attchment_data)
				{		
					$view_attchment.='<a target="blank" href="'.content_url().'/uploads/school_assets/'.$attchment_data.'" class="btn btn-default"><i class="fa fa-download"></i>'. esc_attr__('View Attachment','school-mgt').'</a>';
				}
				$row[7] =$view_attchment;
			}
			else
			{
				 $row[7] = esc_attr__('No Attachment','school-mgt');
			}
			$created_date=$content_post->post_date_gmt;
			
			$row[8] = mj_smgt_convert_date_time($created_date);
			
			$row[9] = '<a href="?page=smgt_message&tab=view_all_message&action=delete_users_message&users_message_id='.$aRow['message_id'].'" class="btn btn-danger" onclick="return confirm('. esc_attr__("'Are you sure you want to delete this message?'","'school-mgt'").')">'. esc_attr__('Delete','school-mgt').'</a>';
...
<?php
	...
	<div class="modal-header">
		<a href="#" class="close-btn-cat badge badge-success float-end">X</a>
		<h4 class="modal-title"><?php esc_attr_e('Student Library History','school-mgt');?></h4>
	</div>
		...
		if(!empty($booklist)){ ?>
			...
			<table class="table" width="100%" style="border-collapse:collapse;">
				<thead>
					<tr>
						<th></th>
						<th class="text-left"><?php esc_attr_e('Book name','school-mgt');?></th>
						<th class="text-left"><?php esc_attr_e('Overdue By','school-mgt');?></th>
						<th class="text-left"> <?php esc_attr_e('Fine','school-mgt');?></th>
					</tr>
				</thead>
				<tbody>
				<?php 
					...
					<tr>
						...
						<td><?php if ($date1 > $date2) echo $diff->format("%a "). esc_attr__("Days","school-mgt"); else echo esc_attr__("0 Days","school-mgt");?></td>
						<td><input type="number" min="0" class="validate[required,min[0],maxSize[5]] number" onkeypress="return isNumberKey(event)" name="fine[]" value=""> </td>
					</tr>
				<?php } ?>
				</tbody>
					<tr><td colspan="4"><input type="submit" class="btn btn-success" name="submit_book" value="<?php esc_attr_e("Submit Book","school-mgt");?>"></td></tr>
			</table>
			</form>
			<?php }
				else
				{
					esc_attr_e('No Any Book Pending','school-mgt');
				}?>
	</div>
	</div>
	<?php 
...
<div class="modal-header"> <a href="#" class="close-btn badge badge-success float-end">X</a>
...
<div class="panel panel-white">
  ...
  <?php 
	...
	{ ?>
	  ...
	  <div id="accordionExample" class="accordion" aria-multiselectable="true" role="tablist">
		<?php 
		$i=0;
		foreach ($all_exam as $exam) /* #### ALL EXAM LOOP STARTS  */
		{
			...
			<div class="mt-2 accordion-item">
				<h4 class="accordion-header" id="heading_<?php echo $i;?>"> 
					<button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_<?php echo $i;?>" aria-expanded="true" aria-controls="heading_<?php echo $i;?>">
				  <?php esc_attr_e('Exam Results','school-mgt'); ?>
			  ...
			  <div id="collapse_<?php echo $i;?>" class="accordion-collapse collapse" aria-labelledby="heading_<?php echo $i;?>" data-bs-parent="#accordionExample">
				...
				<?php
					...
					{ ?>
						<div class="print-button pull-right" style="margin-right: 35px;"> <a href="?page=smgt_student&print=pdf&student=<?php echo $uid;?>&exam_id=<?php echo $exam_id;?>" target="_blank" class="btn btn-info"><?php esc_attr_e('PDF','school-mgt'); ?></a> <a href="?page=smgt_student&print=print&student=<?php echo $uid;?>&exam_id=<?php echo $exam_id;?>" target="_blank" class="btn btn-info"><?php esc_attr_e('Print','school-mgt'); ?></a> </div>
						<?php
					}
					else 
					{ ?>
						<div class="print-button pull-right" style="margin-right: 40px;"> <a href="?dashboard=user&page=student&print=pdf&student=<?php echo $uid;?>&exam_id=<?php echo $exam_id;?>" target="_blank" class="btn btn-info"><?php esc_attr_e('PDF','school-mgt'); ?></a> <a href="?dashboard=user&page=student&print=print&student=<?php echo $uid;?>&exam_id=<?php echo $exam_id;?>" target="_blank" class="btn btn-info"><?php esc_attr_e('Print','school-mgt'); ?></a> </div>
						<?php 				
					} ?>
					<div class="clearfix"></div>
					  ...
					  <div class="table-responsive">				  
						<table class="table table-bordered">
						  <tr>
							...
							<th><?php esc_attr_e('Grade Comment','school-mgt')?></th>
						  </tr>
						  <?php		
							$total=0;
							$grade_point = 0; 
							foreach($subject as $sub) /*** ####  SUBJECT LOOPS STARTS **/
							{
							  ...
							  <tr>
								...
								<td><?php echo $obj_mark->mj_smgt_get_grade_marks_comment($obj_mark->mj_smgt_get_grade($exam_id,$class_id,$sub->subid,$uid));?></td>
							  </tr>
							  <?php
								$total +=  $marks;
						  ...
						  <p class="result_total">
							<?php esc_attr_e("Total Marks","school-mgt"); echo "=> ".$total; ?>
						  </p>
						  <hr />
	...
	{
		esc_attr_e('No Result Found','school-mgt');
	}
	exit;
}
...
<div class="panel panel-white">
   ...
   <form name="expense_form" action="" method="post" class="form-horizontal" id="expense_form">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-3 control-label col-form-label text-md-end" for="roll_id"><?php esc_attr_e('Roll No.','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
		...
		<div >
			<div class="mb-3 form-group row">
				<label class="col-sm-3 control-label col-form-label text-md-end " for="enable"><?php esc_attr_e('Send Mail','school-mgt');?></label>
				<div class="col-sm-2" style="padding-left: 0px;">
					<div class="checkbox">
						<label>
							<input id="chk_sms_sent1" type="checkbox" <?php $smgt_student_mail_service_enable = 0;if($smgt_student_mail_service_enable) echo "checked";?> value="1" name="smgt_student_mail_service_enable">
						</label>
					</div>				 
				</div>
			</div>				
			...
			<div class="mb-3 form-group row">
				<label class="col-sm-3 control-label col-form-label text-md-end " for="enable"><?php esc_attr_e('Send SMS','school-mgt');?></label>
				<div class="col-sm-2" style="padding-left: 0px;">
					<div class="checkbox">
						<label>
							<input id="chk_sms_sent1" type="checkbox" <?php $smgt_studnet_sms_service_enable = 0;if($smgt_studnet_sms_service_enable) echo "checked";?> value="1" name="smgt_studnet_sms_service_enable">
						</label>
					</div>				 
				</div>
			</div>
		...
		<div class="offset-sm-2 col-sm-8">
        	 <input type="submit" value="<?php esc_attr_e('Active Student','school-mgt');?>" name="active_user" class="btn btn-success"/>
        </div>
   </form>
</div>
  <?php
...
function mj_smgt_ajax_smgt_view_notice()
{
	 ...
	 ?>
<div class="form-group"> <a class="close-btn badge badge-success float-end" href="#">X</a>
...
<div class="panel panel-white form-horizontal view_notice_overflow">
  <div class="mb-3 form-group row dis_flex_res">
    <label class="col-sm-3" for="notice_title">
    <?php esc_attr_e('Notice Title','school-mgt');?>
    : </label>
    <div class="col-sm-9"> <?php echo $notice->post_title;?> </div>
  </div>
  <div class="mb-3 form-group row dis_flex_res">
    <label class="col-sm-3" for="notice_title">
    <?php esc_attr_e('Notice Comment','school-mgt');?>
    : </label>
    <div class="col-sm-9"> <?php echo $notice->post_content;?> </div>
  </div>
  <div class="mb-3 form-group row dis_flex_res">
    <label class="col-sm-3" for="notice_title">
    <?php esc_attr_e('Notice For','school-mgt');?>
    : </label>
    <div class="col-sm-9">
    <?php echo esc_attr_e(get_post_meta( $notice->ID, 'notice_for',true),'school-mgt');?>
	 </div>
  </div>
  <div class="mb-3 form-group row dis_flex_res">
    <label class="col-sm-3" for="notice_title">
    <?php esc_attr_e('Start Date','school-mgt');?>
    : </label>
    <div class="col-sm-9"> <?php echo mj_smgt_getdate_in_input_box(get_post_meta( $notice->ID, 'start_date',true));?> </div>
  </div>
  <div class="mb-3 form-group row dis_flex_res">
    <label class="col-sm-3" for="notice_title">
    <?php esc_attr_e('End Date','school-mgt');?>
    : </label>
    <div class="col-sm-9"> <?php echo mj_smgt_getdate_in_input_box(get_post_meta( $notice->ID, 'end_date',true));?> </div>
  </div>
</div>
<?php 
	die();
...
function mj_smgt_student_invoice_view()
{
	...
	<div class="modal-body" style="height:520px; overflow:hidden;">
		<div id="invoice_print"> 
				...
				<tbody>
					<tr>
						...
						<td align="right" width="24%">
							...
							echo esc_attr__('Issue Date','school-mgt')." : ".mj_smgt_getdate_in_input_box(date("Y-m-d", strtotime($issue_date)));?></h5><br>
							<h5><?php 
								if($payment_status=='Paid') 
								{ echo esc_attr__('Status','school-mgt')." : ". esc_attr__('Paid','school-mgt');}
								if($payment_status=='Part Paid')
								{ echo esc_attr__('Status','school-mgt')." : ". esc_attr__('Partially Paid','school-mgt');}
								if($payment_status=='Unpaid')
								{echo esc_attr__('Status','school-mgt')." : ". esc_attr__('Unpaid','school-mgt'); } ?></h5>
						</td>
					</tr>
				</tbody>
			</table>
			<hr class="color_black">
			<table width="100%" border="0">
				<tbody>
					<tr>
						<td class="col-md-6 padding_payment">
						
							<h4><?php if($_POST['invoice_type']=='expense'){ esc_attr_e('Payment From','school-mgt'); }else { esc_attr_e('Payment From','school-mgt'); }?> </h4>
						</td>
			...
			<hr class="color_black">
			<h4><?php esc_attr_e('Invoice Entries','school-mgt');?></h4>
			<div class="table-responsive">
				<table class="table table-bordered" width="100%" border="1" style="border-collapse:collapse;">
					<thead>
						<tr>
							<th class="text-center">#</th>
							<th class="text-center"> <?php esc_attr_e('Date','school-mgt');?></th>
							<th width="60%"><?php esc_attr_e('Entry','school-mgt');?> </th>
							<th><?php esc_attr_e('Price','school-mgt');?></th>
							<th class="text-center"> <?php esc_attr_e('Issue By','school-mgt');?> </th>
						</tr>
					</thead>
					<tbody>
					<?php 
						...
						<tr>
							...
							<td class="text-right"> <?php echo "<span> ". mj_smgt_get_currency_symbol() ."</span>" .number_format($each_entry->amount,2) ; ?></td>
							<td class="text-center"><?php echo mj_smgt_get_display_name($result_income->create_by);?></td>
						</tr>
							<?php $id+=1;}
							}
					 ...
					 if(!empty($invoice_data)){
						...
						<tr>
							...
							<td class="text-right"> <?php echo "<span> ". mj_smgt_get_currency_symbol() ." </span>" .number_format($invoice_data->amount,2); ?></td>
							<td class="text-center"><?php echo mj_smgt_get_display_name($invoice_data->payment_reciever_id);?></td>
						</tr>
						<?php }?>
					</tbody>
				</table>
			...
			<table width="100%" border="0">
				<tbody>							
					...
					?>								
					<tr>
						<td width="80%" align="right"><?php esc_attr_e('Grand Total :','school-mgt');?></td>
						<td align="right"><h4><?php echo "<span> ". mj_smgt_get_currency_symbol() ." </span>" .number_format($grand_total,2); ?></h4></td>
					</tr>
				</tbody>
			</table>
		</div>
		<div class="print-button align-center">
			<a  href="?page=smgt_payment&print=print&invoice_id=<?php echo $_POST['idtest'];?>&invoice_type=<?php echo $_POST['invoice_type'];?>" target="_blank"class="btn btn-success"><?php esc_attr_e('Print','school-mgt');?></a>
			&nbsp;&nbsp;&nbsp;
			<a  href="?page=smgt_payment&print=pdf&invoice_id=<?php echo $_POST['idtest'];?>&invoice_type=<?php echo $_POST['invoice_type'];?>" target="_blank"class="btn btn-success"><?php esc_attr_e('PDF','school-mgt');?></a>
		</div>
	</div>
	<?php 
	 die();
...
?>
	...
	<div class="modal-body">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-3 control-label col-form-label text-md-end" for="amount"><?php esc_attr_e('Paid Amount','school-mgt');?>(<?php echo mj_smgt_get_currency_symbol();?>)<span class="require-field">*</span></label>
			<div class="col-sm-8">
		...
		<div class="mb-3 form-group row">
			...
			<label class="col-sm-3 control-label col-form-label text-md-end" for="payment_method"><?php esc_attr_e('Payment By','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
		...
		<div class="offset-sm-2 col-sm-8">
        	 <input type="submit" value="<?php esc_attr_e('Add Payment','school-mgt');?>" name="add_feetype_payment" class="btn btn-success"/>
        </div>
		</form>
	</div>
<?php
				...
				<tr>
					...
					<td align="right" width="24%">
						...
						$issue_date='DD-MM-YYYY';
							...
							<h5><?php echo esc_attr__('Status','school-mgt')." : "; echo "<span class='btn btn-success btn-xs'>";
...
{ 
	...
	<style>
		@page{
			background-image: url("<?php echo SMS_PLUGIN_URL .'/assets/images/Invoice-BG.png' ?>") ;
		}	
	</style>
	<div class="modal-body invoice_bg_image">
	...
	<table class="table table-bordered" width="100%" border="1" style="border-collapse:collapse;">
		<thead>
			<tr>
				<th class="text-center">#</th>
				<th class="text-center"><?php esc_attr_e('Date','school-mgt');?></th>
				<th class="text-center"> <?php esc_attr_e('Fees Type','school-mgt');?></th>
		...
		?>
		 <tbody>
			<tr>
				<td> <?php echo $x; ?></td>
				<td> <?php echo mj_smgt_getdate_in_input_box($fees_detail_result->created_date);?></td>
				<td><?php echo mj_smgt_get_fees_term_name($id);?></td>
				<td><?php 
				$obj_feespayment= new mj_smgt_feespayment();
				$amount=$obj_feespayment->mj_smgt_feetype_amount_data($id);
	...
	<table width="100%" border="0">
		<tbody>
			<tr>
				<td width="80%" align="right"><?php esc_attr_e( 'Sub Total :','school-mgt' ); ?></td>
				<td align="right"><?php echo "<span> ". mj_smgt_get_currency_symbol() ." </span>" .number_format($fees_detail_result->total_amount,2); ?></td>
			</tr>
			<tr>
				<td width="80%" align="right"><?php esc_attr_e('Payment Made :','school-mgt');?></td>
				<td align="right"><?php echo "<span> ". mj_smgt_get_currency_symbol() ." </span>" . number_format($fees_detail_result->fees_paid_amount,2) ;?></td>
			</tr>
			<tr>
				<td width="80%" align="right"><?php esc_attr_e( 'Due Amount :','school-mgt' ); ?></td>
				<?php $Due_amount = $fees_detail_result->total_amount - $fees_detail_result->fees_paid_amount;?>
				<td align="right"><?php echo "<span> ". mj_smgt_get_currency_symbol() ." </span>" . number_format($Due_amount,2) ?></td>
			</tr>
		</tbody>
	</table>
	<hr>
	<?php if(!empty($fees_history_detail_result)){ ?>
		...
		<table class="table table-bordered" width="100%" border="1" style="border-collapse:collapse;">
			<thead>
				<tr>
					<th class="text-center"><?php esc_attr_e('Date','school-mgt');?></th>
					<th class="text-center"> <?php esc_attr_e('Amount','school-mgt');?></th>
					<th><?php esc_attr_e('Method','school-mgt');?> </th>				
				</tr>
			</thead>
			<tbody>
				<?php 
...
function mj_smgt_student_view_librarryhistory()
{
	...
	<div id="invoice_print"> 
					...
					<thead>
							<tr>
								<th class="text-center"><?php esc_attr_e('Book name','school-mgt');?></th>
								<th class="text-center"> <?php esc_attr_e('Issue Date','school-mgt');?></th>
								<th class="text-center"> <?php esc_attr_e('Return Date','school-mgt');?></th>
								<th><?php esc_attr_e('Period','school-mgt');?> </th>
								<th><?php esc_attr_e('Overdue By','school-mgt');?> </th>
						...
						<tbody>
							...
							<td><?php echo get_the_title($retrieved_data->period). esc_attr__(" Days","school-mgt");?></td>
							<?php 
						...
						?>
							...
							elseif($date1 > $date2)
							{ 
								echo $diff->format("%a"). esc_attr__("0 Days","school-mgt"); 
							}?></td>
							<td><?php echo  ($retrieved_data->fine != "" || $retrieved_data->fine != 0) ? mj_smgt_get_currency_symbol().$retrieved_data->fine : "NA";?></td>
							</tr>
							<?php }?>
						</tbody>
					</table>
...
function mj_smgt_add_category_type($model,$class_id) 
{
	...
	if($model == 'class_sec')
	{
		...
		$button_text=  esc_attr__("Add Section Name",'school-mgt');
		$label_text =  esc_attr__("Section Name",'school-mgt');
	}
	?>
	<div class="modal-header"> <a href="#" class="close-btn-cat badge badge-success float-end">X</a>
	...
	<div class="panel panel-white">
  	...
  	<table class="table">
  	 ...
  	 <form name="fee_form" action="" method="post" class="form-horizontal" id="fee_form">
  	 	<div class="mb-3 form-group row" style="margin-top: 10px;">
			<label class="col-sm-4 control-label col-form-label text-md-end" for="fee_type"><?php echo $label_text;?><span class="require-field">*</span></label>
			<div class="col-sm-4">
				<?php 
				if($model == 'period_type')
				{
				?>					
					<input id="txtfee_type" class="form-control text-input validate[required]" maxlength="3" type="number" value=""  oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" name="txtfee_type" placeholder="<?php esc_attr_e('Must Be Enter Number of Days','school-mgt');?>">
				<?php
				}
				else
				{
...
function mj_smgt_add_fee_type()
{
	...
	if($model=='period_type')
	{
		...
		$id = $wpdb->insert_id;
		$row1 = '<tr id="cat-'.$id.'"><td>'.stripslashes($_REQUEST['fee_type']).' '. esc_attr__('Days','school-mgt').'</td><td><a class="btn-delete-cat badge badge-delete" href="#" id='.$id.'>X</a></td></tr>';
...
function mj_smgt_edit_section()
{
	...
	//echo '<td>'.$i.'</td>';
					...
					echo '<td id='.$retrieved_data->id.'>
					<a class="btn-cat-update-cancel btn btn-danger" model='.$model.' href="#" id='.$retrieved_data->id.'>'. esc_attr__('Cancel','school-mgt').'</a>
					<a class="btn-cat-update btn btn-primary" model='.$model.' href="#" id='.$retrieved_data->id.'>'. esc_attr__('Save','school-mgt').'</a>
					</td>';
	die();
}
function mj_smgt_student_invoice_print($invoice_id)
{
	...
	<style>
		@media print {
			.background_image_print{
				background : url( '../images/Invoice-BG.png' );
				height:100%;
				width: 100%;
			}	
			* {
	...
	<?php
		...
		<div class="modal-body" style="direction: rtl;">
			...
			<div class="modal-header">
				...
				<table width="100%" border="0">
								...
								<tr>
									...
									<td align="right" width="24%">
										...
										elseif($payment_status=='Part Paid')
											echo esc_attr__('Status','school-mgt')." : ". esc_attr__('Part Paid','school-mgt');
										else
											echo esc_attr__('Status','school-mgt')." : ". esc_attr__('Unpaid','school-mgt');	?></h5>
									</td>
								</tr>
						...
						<hr>
						<h4><?php esc_attr_e('Invoice Entries222','school-mgt');?></h4>
						<table class="table table-bordered" width="100%" border="1" style="border-collapse:collapse;">
							<thead>
								<tr>
									<th class="text-center">#</th>
									...
									<th width="60%"><?php esc_attr_e('Entry','school-mgt');?> </th>
									<th><?php esc_attr_e('Price','school-mgt');?></th>
									<th class="text-center"> <?php esc_attr_e('Username','school-mgt');?> </th>
								</tr>
							</thead>
							<tbody>
							<?php 
						...
						<table width="100%" border="0">
							<tbody>
								...
								<tr>
									...
									<td align="right"><h4><?php esc_attr_e('Grand Total :','school-mgt');?><?php echo "<span>". mj_smgt_get_currency_symbol() ."</span>" .number_format($grand_total,2); ?></h4></td>
								</tr>
							</tbody>
						</table>
			</div>
		...
		?>
			<div class="modal-body background_image_print" style="background-image: url(<?php echo SMS_PLUGIN_URL .'/assets/images/Invoice-BG.png' ?>);width: 100%;height:100%;">
			<div id="invoice_print" style="width: 90%;margin:0 auto;"> 
			<div class="modal-header">
...
function mj_smgt_change_profile_photo()
{
	...
	<div class="mb-3 form-group row">
	<label for="inputEmail" class="control-label col-form-label text-md-end col-sm-3"><?php esc_attr_e('Select Profile Picture','school-mgt');?></label>
		<div class="col-xs-8">	
			<input id="input-1" name="profile" type="file" onchange="mj_smgt_fileCheck(this);" style="border:0px;"  class="form-control file profile_file">
		</div>
	</div>
	<div class="mb-3 form-group row">
		<div class="col-xs-offset-2 col-sm-10" style="margin-bottom: 5px;">
				<button type="submit" class="btn btn-success" name="save_profile_pic"><?php esc_attr_e('Save','school-mgt');?></button>
		</div>
	</div>
	</form>
    <?php 
...
?>
     <div class="modal-header model_header_padding"> <a href="#" class="event_close-btn badge badge-success float-end">X</a>
  		<h4 id="myLargeModalLabel" class="modal-title"><?php if($model=='Notification Details'){ esc_attr_e('Notification Details','school-mgt'); } elseif($model=='Noticeboard Details'){ esc_attr_e('Notice Board Details','school-mgt'); } elseif($model=='Exam Details'){ esc_attr_e('Exam Details','school-mgt'); } elseif($model=='holiday Details'){ esc_attr_e('holiday Details','school-mgt'); }?></h4>
	</div>
	<div class="panel panel-white">
	<?php
	if($model=='Notification Details')
	...
	?>
		<div class="modal-body">
			<table id="examlist" class="table table-striped" cellspacing="0" width="100%" align="center">
				<tbody>
					<tr>
						<td><?php esc_attr_e('Student Name','school-mgt'); ?></td>
						<td><?php echo mj_smgt_get_user_name_byid($notification_data->student_id); ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Title','school-mgt'); ?></td>
						<td><?php echo $notification_data->title; ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Message','school-mgt'); ?></td>
	...
	?>
		<div class="modal-body">
			<table id="examlist" class="table table-striped" cellspacing="0" width="100%" align="center">
				<tbody>
					<tr>
						<td><?php esc_attr_e('Notice Title','school-mgt'); ?></td>
						<td><?php echo $retrieve_class->post_title; ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Notice Comment','school-mgt'); ?></td>
					...
					<tr>
						<td><?php esc_attr_e('Notice Start Date','school-mgt'); ?></td>
						<td><?php echo mj_smgt_getdate_in_input_box(get_post_meta($retrieve_class->ID,'start_date',true)); ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Notice End Date','school-mgt'); ?></td>
						<td><?php echo mj_smgt_getdate_in_input_box(get_post_meta($retrieve_class->ID,'end_date',true)); ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Notice For','school-mgt'); ?></td>
	...
	?>
		<div class="modal-body">
			<table id="examlist" class="table table-striped" cellspacing="0" width="100%" align="center">
				<tbody>
					<tr>
						<td><?php esc_attr_e('Exam Title','school-mgt'); ?></td>
						<td><?php echo $exam_data->exam_name; ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Exam Start Date','school-mgt'); ?></td>
						<td><?php echo mj_smgt_getdate_in_input_box($exam_data->exam_start_date); ?>
						</td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Exam End Date','school-mgt'); ?></td>
						<td><?php echo mj_smgt_getdate_in_input_box($exam_data->exam_end_date); ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Exam Comment','school-mgt'); ?></td>
	...
	?>
		<div class="modal-body">
			<table id="examlist" class="table table-striped" cellspacing="0" width="100%" align="center">
				<tbody>
					<tr>
						<td><?php esc_attr_e('Holiday Title','school-mgt'); ?></td>
						<td><?php echo $holiday_data->holiday_title; ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Description','school-mgt'); ?></td>
						<td><?php echo $holiday_data->description; ?></td>
					</tr>
					<tr>
						<td><?php esc_attr_e('Start Date','school-mgt'); ?></td>
						<td><?php echo mj_smgt_getdate_in_input_box($holiday_data->date); ?>
						</td>
					</tr>
					<tr>
						<td><?php esc_attr_e('End Date','school-mgt'); ?></td>
						<td><?php echo mj_smgt_getdate_in_input_box($holiday_data->end_date); ?></td>
					</tr>
...
{	 
	...
	if($model == 'period_type')//term_category
	{
		...
		$button_text=  esc_attr__("Add Period Time",'school-mgt');
   ...
   jQuery('.onlyletter_number').keypress(function(e) 
	...
	<div class="modal-header" style="padding:15px!important;"> <a href="#" class="close-btn badge badge-success float-end">X</a><!--MODAL-HEADER--->
	...
	<div class="panel panel-white"><!---PANEL-WHITE--->
		  		...
		  		<table class="table">
					...
					{			
						foreach ($cat_result1 as $retrieved_data)
						{
							...
							echo '<td>'.$retrieved_data->post_title.' '. esc_attr__("Days","school-mgt").'</td>';
							echo '<td id='.$retrieved_data->ID.'><a class="px-2 py-1 btn-delete-cat_new badge badge-delete" model='.$model.' href="#" id='.$retrieved_data->ID.'>X</a></td>';
							echo '</tr>';
							$i++;		
						}
					}
					else
					{
						foreach ($cat_result as $retrieved_data)
						{
							echo '<tr id="cat_new-'.$retrieved_data->ID.'">';
							echo '<td>'.$retrieved_data->post_title.'</td>';
							echo '<td id='.$retrieved_data->ID.'><a class="px-2 py-1 btn-delete-cat_new badge badge-delete" model='.$model.' href="#" id='.$retrieved_data->ID.'>X</a><a class="btn-edit-cat_popup badge badge-edit"  model="'.$model.'" href="#" id="'.$retrieved_data->ID.'"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td>';
							echo '</tr>'; 
							$i++;		
						}
					}  
		...
		<form name="category_form" action="" method="post" class="form-horizontal" id="category_form_test"><!---CATEGORY_FORM----->
	  	 	<div class="mb-3 form-group row">
				<label class="col-sm-4 control-label col-form-label text-md-end" for="category_name"><?php echo $label_text;?><span class="require-field">*</span></label>
				<div class="col-sm-4" style="padding-bottom: 10px;">
					<?php					
					if($model == 'period_type')
					{
					?>					
						<input id="category_name" maxlength="50" min="1" class="cat_value form-control text-input onlyletter_number" type="number"  value=""  name="category_name"  placeholder="<?php esc_attr_e('Must Be Enter Number of Days','school-mgt');?>">
					<?php
					}
					else
					{
					?>
						<input id="category_name" maxlength="50" class="cat_value form-control text-input onlyletter_number_space_validation" value="" name="category_name" <?php if(isset($placeholder_text)){?> type="number" placeholder="<?php  echo $placeholder_text;}else{?> type="text" <?php }?>>
				...
				<div class="col-sm-4" style="padding-bottom: 10px;">
					<input type="button" value="<?php echo $button_text;?>" name="save_category_test" class="btn btn-success" model="<?php echo $model;?>" id="btn_add_cat_new_test">
				</div>
			</div>
		</form>
...
function mj_smgt_add_category_new($data)
{
	...
	if($model == 'period_type')
	{
		$row1 = '<tr id="cat_new-'.$id.'"><td>'.$_REQUEST['category_name'].' '. esc_attr__("Days","school-mgt").'</td><td><a class="px-2 py-1 btn-delete-cat_new badge badge-delete" href="#" id='.$id.' model="'.$model.'">X</a></td></tr>';
		$option = "<option value='$id'>".$_REQUEST['category_name'].' Days'."</option>";
	}
	else
	{
		$row1 = '<tr id="cat_new-'.$id.'"><td>'.$_REQUEST['category_name'].'</td><td><a class="px-2 py-1 btn-delete-cat_new badge badge-delete" href="#" id='.$id.' model="'.$model.'">X</a><a class="btn-edit-cat_popup badge badge-edit"  model="'.$model.'" href="#" id="'.$id.'"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td></tr>';
...
<div class="panel panel-white admission_div_responsive">
   ...
   <form name="admission_form" action="" method="post" class="form-horizontal" id="admission_form">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="roll_id"><?php esc_attr_e('Roll No.','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
		...
		<div class="offset-sm-2 col-sm-8">
        	 <input type="submit" value="<?php esc_attr_e('Active Student','school-mgt');?>" name="active_user_admission" class="btn btn-success margin_top_20"/>
        </div>
   </form>
</div>
  <?php
	 ...
	 if ( isset( $_REQUEST['iDisplayStart'] ) && $_REQUEST['iDisplayLength'] != '-1' )
	 {
	   ...
	   $sQuery = "SELECT * FROM $sTable";
		 ...
		 foreach($rResult as $aRow)
		 {
			...
			$row[5]=$aRow['to_date'];
			$row[6] = '
		    <a href="?page=smgt_homework&tab=addhomework&action=edit&homework_id='.$aRow['homework_id'].'" class="btn btn-info"><i class="fa fa-pencil-square-o"></i>&nbsp; '. esc_attr__("Edit","school-mgt").' </a>&nbsp;&nbsp;
...
{  
	...
	<form name="receipt_form" action="" method="post" class="form-horizontal" id="receipt_form">
		...
		<div class="mb-3 form-group row">
					...
					<div class="table-responsive">
						<table class="table exam_hall_table" id="exam_hall_table" style="border: 1px solid #000000;text-align: center;margin-bottom: 0px;border-collapse: separate;">
							<thead>
								<tr>
									<th  style="border-top: medium none;border-right: 1px solid #000000;background-color: #e5e5e5;border-bottom: 1px solid #000000;text-align: center;">'. esc_attr__('Exam','school-mgt').'</th>
									<th  style="border-right: 1px solid #000000;background-color: #e5e5e5;border-bottom: 1px solid #000000;text-align: center;">'. esc_attr__('Class','school-mgt').'</th>							
									<th  style="border-right: 1px solid #000000;background-color: #e5e5e5;border-bottom: 1px solid #000000;text-align: center;">'. esc_attr__('Section','school-mgt').'</th>							
									<th  style="border-right: 1px solid #000000;background-color: #e5e5e5;border-bottom: 1px solid #000000;text-align: center;">'. esc_attr__('Term','school-mgt').'</th>							
									<th  style="border-right: 1px solid #000000;background-color: #e5e5e5;border-bottom: 1px solid #000000;text-align: center;">'. esc_attr__('Start Date','school-mgt').'</th>							
									<th  style="background-color: #e5e5e5;border-bottom: 1px solid #000000;text-align: center;">'. esc_attr__('End Date','school-mgt');'</th>							
								</tr>
							</thead>
							<tfoot></tfoot>
							<tbody>';						
								$array_var.='<tr>
									...
									<td style="border-right: 1px solid #000000;">'.mj_smgt_get_class_name($exam_data->class_id);
									$array_var.='</td>
									...
									$array_var.='</td>
									<td style="border-right: 1px solid #000000;">'. mj_smgt_getdate_in_input_box($start_date);
									$array_var.='</td>
									<td style="">'.mj_smgt_getdate_in_input_box($end_date);
									$array_var.='</td>
								</tr>
							</tbody>
						</table>
		...
		<div class="mb-3 form-group row">
			<label for="exam_id" class="col-md-2 col-sm-2 col-xs-12 col-form-label text-md-end">'. esc_attr__('Select Exam Hall','school-mgt').'<span class="require-field">*</span></label>
			...
			<div class="col-md-12">
				<div class="row">';
					...
					{  
						...
						$array_var.='<h4>'. esc_attr__('Not Assign Exam Hall Student List' , 'school-mgt').'</h4>';
						if(isset($student_show_data))
						{
							$array_var.='<table id="not_approve_table" class="display exam_timelist" cellspacing="0" width="100%">
								<thead>
									<tr>
										...
										<th>'. esc_attr__('Student Name' , 'school-mgt').'</th>
										<th>'. esc_attr__('Student Roll No', 'school-mgt').'</th>
									</tr>
								</thead>
								<tfoot>
									<tr>					
							...
							<tr> 
								<td> 
									<button type="button" class="btn btn-success assign_exam_hall" name="assign_exam_hall" id="assign_exam_hall">'. esc_attr__('Assign Exam Hall', 'school-mgt').'</button>
								</td>
							</tr>';
						}
					$array_var.='</div>';
					$array_var.="<div class='col-md-6 col-sm-6 col-xs-12'>";
						$array_var.='<h4>'. esc_attr__('Assign Exam Hall Student List' , 'school-mgt').'</h4>';
						...
						{  
							$array_var.='<table id="approve_table" class="display exam_timelist" cellspacing="0" width="100%">
								...
								{
									$array_var.='<td  class="no_data_td_remove1" style="text-align:center;" colspan="3">'. esc_attr__('No Student Available' , 'school-mgt').'</td>';
								}
								$array_var.='</tbody>
						</table>
							<tr>
								<td>
									<button type="submit" class="btn btn-success" name="send_mail_exam_receipt" id="send_mail_exam_receipt">'. esc_attr__('Send Mail', 'school-mgt').'</button>
								</td>
							</tr>';
						}
					$array_var.='</div>';
					...
					{
						$array_var.='<div><h4 class="text-danger">'. esc_attr__('No Student Available', 'school-mgt').'</h4></div>';
					}
				$array_var.='</div>
			</div>
		</div>
...
<?php
	...
	{ 		
		...
		<div class="form-group"> 	
			<a href="#" class="close-btn badge badge-success float-end">X</a>
			<h4 class="modal-title" id="myLargeModalLabel">
				<?php esc_attr_e('Transport Details','school-mgt'); ?>
			</h4>
		</div>
		<hr>		
		<div class="panel-body view_details_popup_body view_transport_details_popup">
			<table id="transport_list" class="table table-striped" cellspacing="0" width="100%" align="center">
				<tbody>
					<tr>
						<td width="40%">
							<b><?php esc_attr_e('Route Name','school-mgt'); ?></b> : 
						</td>
						<td width="60%">
					...
					<tr>
						<td width="40%">
							<b><?php esc_attr_e('Vehicle Identifier','school-mgt'); ?></b> :
						</td>
						<td width="60%">
					...
					<tr>
						<td width="40%">
							<b><?php esc_attr_e('Vehicle Registration Number','school-mgt'); ?></b> :
						</td>
                        <td width="60%">
				...
				<tr>
					<td width="40%">
						<b><?php esc_attr_e('Driver Name','school-mgt'); ?></b> :
					</td>
					<td width="70%">
				...
				<tr>
					<td width="40%">
						<b><?php esc_attr_e('Driver Phone Number','school-mgt'); ?></b> :
					</td>
					<td width="60%">
				...
				<tr>
					<td width="40%">
						<b><?php esc_attr_e('Driver Address','school-mgt'); ?></b> :
					</td>
					<td width="60%">
				...
				<tr>
					<td width="40%">
						<b><?php esc_attr_e('Route fare','school-mgt'); ?></b> :
					</td>
					<td width="60%">
						<?php echo "<span> ". mj_smgt_get_currency_symbol() ." </span>" . number_format($transport_data->route_fare,2); ?>
					</td>
				</tr>
				
				<tr>
					<td width="40%">
						<b><?php esc_attr_e('Route Description','school-mgt'); ?></b> : 
					</td>
					<td width="60%">
	...
	{ 	
		...
		<div class="form-group"> 	
			<a href="#" class="close-btn badge badge-success float-end">X</a>
			<h4 class="modal-title" id="myLargeModalLabel">
				<?php esc_attr_e('Book Details','school-mgt'); ?>
			</h4>
		</div>
		<hr>		
		<div class="panel-body view_details_popup_body">
			<table id="transport_list" class="table table-striped" cellspacing="0" width="100%" align="center">
				<tbody>
					<tr>
						<td>
							<b><?php esc_attr_e('ISBN','school-mgt'); ?></b> : &nbsp;
							<?php echo $book_data->ISBN; ?>
						</td>
					</tr>
					<tr>
						<td>
							<b><?php esc_attr_e('Book Name','school-mgt'); ?></b> : &nbsp;
							<?php 					
					...
					<tr>
						<td>
							<b><?php esc_attr_e('Book Category','school-mgt'); ?></b> : &nbsp;
							<?php 					
					...
					<tr>
						<td>
							<b><?php esc_attr_e('Author Name','school-mgt'); ?></b> : &nbsp;
				...
				<tr>
					<td>
						<b><?php esc_attr_e('Rack Location','school-mgt'); ?></b> : &nbsp;
				...
				<tr>
					<td>
						<b><?php esc_attr_e('Description','school-mgt'); ?></b> : &nbsp;
...
function mj_smgt_edit_popup_value()
{
	...
	$cat_id = $_REQUEST['cat_id'];
				...
				echo '<td id='.$cat_id.'>
				<a class="btn-cat-update-cancel_popup btn btn-danger" model='.$model.' href="#" id='.$cat_id.'>'. esc_attr__('Cancel','school-mgt').'</a>
				<a class="btn-cat-update_popup btn btn-primary" model='.$model.' href="#" id='.$cat_id.'>'. esc_attr__('Save','school-mgt').'</a>
				</td>';
	die();
}
...
function mj_smgt_access_right_page_not_access_message()
{
	...
	<script type="text/javascript">
		$(document).ready(function() 
		{	
			alert('<?php esc_attr_e('You do not have permission to perform this operation.','hospital_mgt');?>');
...
function mj_smgt_ajax_create_meeting()
{
	...
	<div class="modal-header"> <a href="#" class="close-btn badge badge-success float-end">X</a>
		 <h4 class="margin_0px"><?php esc_attr_e('Create Virtual Class','school-mgt');?></h4> 
	</div>
	<hr class="hr_margin">
	<div class="panel panel-white">
	  	<div class="panel-body">   
        ...
        <div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="class_name"><?php esc_attr_e('Class Name','school-mgt');?></label>
			<div class="col-sm-8">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="class_section"><?php esc_attr_e('Class Section','school-mgt');?></label>
			<div class="col-sm-8">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="Subject"><?php esc_attr_e('Subject','school-mgt');?></label>
			<div class="col-sm-8">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="start_time"><?php esc_attr_e('Start Time','school-mgt');?></label>
			<div class="col-sm-8">
				<input id="start_time" class="form-control" type="text" value="<?php echo $start_time; ?>" name="start_time" disabled>
			</div>
		</div>
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="end_time"><?php esc_attr_e('End Time','school-mgt');?></label>
			<div class="col-sm-8">
				<input id="end_time" class="form-control" type="text" value="<?php echo $end_time; ?>" name="end_time" disabled>
			</div>
		</div>
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="start_date"><?php esc_attr_e('Start Date','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="end_date"><?php esc_attr_e('End Date','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="agenda"><?php esc_attr_e('Topic','school-mgt');?></label>
			<div class="col-sm-8">
		...
		<div class="mb-3 form-group row">
			<label class="col-sm-2 control-label col-form-label text-md-end" for="title"><?php esc_attr_e('Password','school-mgt');?></label>
			<div class="col-sm-8">
		...
		<div class="offset-sm-2 col-sm-8">        	
        	<input type="submit" value="<?php if($edit){ esc_attr_e('Save Virtual Class','school-mgt'); }else{ esc_attr_e('Create Virtual Class','school-mgt');}?>" name="create_meeting" class="btn btn-success" />
        </div>        
     </form>
    </div>
	</div>
...
function mj_smgt_ajax_view_meeting_detail()
{
	...
	<div class="modal-header"> <a href="#" class="close-btn badge badge-success float-end">X</a>
		 <h4><?php esc_attr_e('View Virtual Class Details','school-mgt');?></h4> 
	</div>
	<hr>
	<div class="panel panel-white form-horizontal view_notice_overflow">
		<div class="copy_text">
			<p><?php esc_attr_e('Meeting ID','school-mgt');?> : <?php echo $class_data->zoom_meeting_id; ?></p>
			<p><?php esc_attr_e('Password','school-mgt');?> : <?php echo $class_data->password; ?></p>
			<p><?php esc_attr_e('Join Virtual Class Link','school-mgt');?> : <?php echo $class_data->meeting_join_link; ?></p>
		</div>
...
{
	?>
	<div class="modal-header"><?php  esc_attr_e('Import Data','school-mgt');?><a href="#" class="close-btn-cat badge badge-danger pull-right">X</a>
	</div>
	<form class="form-horizontal" action="#" method="post" enctype="multipart/form-data">
	<div class="mb-3 form-group row">
	<label for="inputEmail" class="control-label col-form-label text-md-end col-sm-2 padding_top_0"><?php  esc_attr_e('Select CSV file','school-mgt');?></label>
		<div class="col-sm-8">	
			<input id="input-1" name="csv_file" type="file" class="form-control file">
		</div>
	</div>
	<div class="mb-3 form-group row">
		<div class="offset-sm-2 col-sm-10 margin_bottom_15">
			<button type="submit" class="btn btn-success" name="upload_csv_file"><?php esc_attr_e('Save','school-mgt');?></button>
		</div>
	</div>
	</form>
    <?php 
