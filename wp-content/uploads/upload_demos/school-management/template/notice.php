...
<?php
	...
	{
		case '1':
			$message_string = esc_attr__('Record Successfully Insert!','school-mgt');
			break;
		case '2':
			$message_string = esc_attr__('Record Successfully Updated!','school-mgt');
			break;	
		case '3':
			$message_string = esc_attr__('Notice Delete Successfully','school-mgt');
			break;
	}
...
<?php
	...
	<ul class="nav nav-tabs panel_tabs" role="tablist">
		<li class="nav-item">
			<a href="?dashboard=user&page=notice&tab=noticelist" class="p-2 px-3 nav-link nav-tab2  <?php echo $active_tab == 'noticelist' ? 'active' : ''; ?>"> <strong>
				<i class="fa fa-align-justify"> </i> <?php esc_attr_e('Notice List', 'school-mgt'); ?></strong>
			</a>
		</li>
		<li class="nav-item">
		<?php if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
		{?>
			<a href="?dashboard=user&page=notice&tab=addnotice&action=edit&notice_id=<?php echo $_REQUEST['notice_id'];?>" class="p-2 px-3 nav-link nav-tab2 <?php echo $active_tab == 'addnotice' ? 'nav-tab-active' : ''; ?>">
		<?php esc_attr_e('Edit Notice', 'school-mgt'); ?></a>  
		<?php 
		}
		else
		{
			...
			{?>
			<a href="?dashboard=user&page=notice&tab=addnotice" class="p-2 px-3 nav-link nav-tab2  <?php echo $active_tab == 'addnotice' ? 'active' : ''; ?>"><?php echo '<span class="fa fa-plus-circle"></span>'.esc_attr__(' Add Notice', 'school-mgt'); ?></a>  
        <?php 
			}
		}?>
		</li>
		...
		<div class="panel-body">
			<div class="table-responsive">
				<table id="notice_list"class="display dataTable notice_datatable" cellspacing="0" width="100%">
					<thead>
						<tr>                
							<th width="190px"><?php esc_attr_e( 'Notice Title', 'school-mgt' ) ;?></th>
							...
							<th width="60px"><?php esc_attr_e( 'Action', 'school-mgt' ) ;?></th>                     
						</tr>
					</thead>
						...
						if (! empty ($notice_list))
						{
							...
							{ ?>
								...
								<tr>
									...
									?></td>
									<td><?php echo mj_smgt_getdate_in_input_box(get_post_meta($retrieved_data->ID,'start_date',true));?></td> 
									<td><?php echo mj_smgt_getdate_in_input_box(get_post_meta($retrieved_data->ID,'end_date',true));?></td> 			      
									<!-- <td><?php print get_post_meta( $retrieved_data->ID, 'notice_for',true);?></td> -->
									<td><?php print esc_attr_e(get_post_meta( $retrieved_data->ID, 'notice_for',true),'school-mgt'); ?></td>
									<td>
									 <?php 
									 ...
									 echo mj_smgt_get_class_name(get_post_meta( $retrieved_data->ID, 'smgt_class_id',true));}?></td>            									
									<td>
										<a href="#" class="btn btn-primary view-notice" id="<?php echo $retrieved_data->ID;?>"> <?php esc_attr_e('View','school-mgt');?></a>
										 <?php
										if($user_access['edit']=='1')
										{
										?>
											<a href="?dashboard=user&page=notice&tab=addnotice&action=edit&notice_id=<?php echo $retrieved_data->ID; ?>" class="btn btn-info"> <?php esc_attr_e('Edit','school-mgt');?></a>
										<?php
										}
	...
	{	 ?>
		...
		<div class="panel-body"> 
			<form name="class_form" action="" method="post" class="mt-3 form-horizontal" id="notice_form">
				...
				<div class="mb-3 form-group row" id="smgt_select_section">
