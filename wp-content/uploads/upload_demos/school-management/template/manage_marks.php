	...
	else
	{
		...
		if($result)
		{
			...
			<div class="alert_msg alert alert-success alert-dismissible " role="alert">
				...
				<p><?php esc_attr_e('Mark Successfully Updated!','school-mgt');?>
			</div>
			<?php 
		}
...
<div class="pt-4 panel-body panel-white">  	
	<ul class="nav nav-tabs panel_tabs" role="tablist">
		  <li class="nav-item ">
			 <a href="?dashboard=user&page=manage_marks&tab=result" class="p-2 px-3 nav-link nav-tab2 <?php echo $active_tab == 'result' ? 'active' : ''; ?>"> <strong>
			 <i class="fa fa-pencil-square-o"> </i> <?php esc_attr_e('Manage Marks', 'school-mgt'); ?></strong></a>
		  </li>
		   <li class="nav-item">
			 <a href="?dashboard=user&page=manage_marks&tab=export_marks" class="p-2 px-3 nav-link nav-tab2 <?php echo $active_tab == 'export_marks' ? 'active' : ''; ?>"> <strong>
			 <i class="fa fa-upload"> </i> <?php esc_attr_e('Export Marks', 'school-mgt'); ?></strong></a>
		  </li>
		  <li class="nav-item">
			 <a href="?dashboard=user&page=manage_marks&tab=multiple_subject_marks" class="p-2 px-3 nav-link nav-tab2 margin_bottom  <?php echo $active_tab == 'multiple_subject_marks' ? 'active' : ''; ?>"> <strong>
			 <i class="fa fa-tasks"> </i> <?php esc_attr_e('Add Multiple Subject Marks', 'school-mgt'); ?></strong></a>
		  </li>
    ...
    <form method="post" id="Add_marks_form">  
		...
		<div class="row">
			<div class="form-group col-md-2">
					...
					<select name="class_id"  id="class_list" class="form-control class_id_exam validate[required] ln-h-28-px" >
			...
			<div class="form-group col-md-2">
				<label for="exam_id"><?php esc_attr_e('Select Exam','school-mgt');?><span class="require-field">*</span></label>
					<select name="exam_id" class="form-control validate[required] exam_list ln-h-28-px" >
						<?php
			...
			<div class="form-group col-md-3">
				<label for="subject_id"><?php esc_attr_e('Select Subject','school-mgt');?><span class="require-field">*</span></label>
				<select name="subject_id" id="subject_list" class="form-control validate[required] ln-h-28-px">
					<?php
    ...
    <div class="form-group col-md-3 button-possition">
    	<label for="subject_id">&nbsp;</label>
      	<input type="submit" value="<?php esc_attr_e('Manage Marks','school-mgt');?>" name="manage_mark"  class="btn btn-success" id="manage_marks"/>
    </div>
   ...
   ?>
	 ...
	 {
		if($role =='teacher')
			$class_id=mj_smgt_get_subject_class($_REQUEST['subject_id']);
		else
		 $class_id =$_REQUEST['class_id'];
    ...
    <label for="file"><?php esc_attr_e('Select CSV file','school-mgt')?>:  </label>   <input type="file" name="csv_file" id="csv_file" class="d-inline"/>
      <input type="submit" name="upload_csv_file" value="<?php esc_attr_e('Fill data from CSV File','school-mgt');?>" class="btn btn-info btn_top" /> 
	   <?php esc_attr_e('CSV file Must have headers as follows','school-mgt');?>: roll_no, name, marks, comment
      <br /><p></p>
	<div class="table-responsive">
		<table class="table col-md-12">
			<tr>
				...
				<th><?php esc_attr_e('Mark Obtained(out of 100)','school-mgt');?></th>
		...
		<?php
			...
			foreach ( $student as $user ) {
				...
				echo '<tr>';
				echo '<td><span '.(isset($csv) && !(isset($key))? 'class="color-red">': '>' ).$user->roll_id. '</span></td>';
				echo '<td><span>' .$user->first_name.' '.$user->last_name. '</span></td>';
				echo '<td>';
				echo '<input type="text" name="marks_'.$user->ID.'" value="'.(isset($csv)&& !(isset($key))? $csv[$key]['marks'] : $marks).'" class="form-control validate[required] text-input">';
				echo '</td>';
	...
	if($access == 1)
	{
		...
		<div class="mt-3 col-sm-12">
			<div class="pull-right">
				<input type="submit" class="btn btn-success" name="save_all_marks" value="<?php esc_attr_e('Update All Marks','school-mgt')?>">
			</div>
	   </div>
		<?php
	}
