...
$feepaydata = $obj_feespayment->mj_smgt_get_single_fee_payment($_REQUEST["fees_pay_id"]);
...
$p->add_field('return', home_url().'/?dashboard=user&page=feepayment'); // return URL after the transaction got over
...
$p->add_field('currency_code', get_option( 'smgt_currency_code' ));
$p->add_field('invoice', date("His").rand(1234, 9632));
$p->add_field('item_name_1', mj_smgt_get_fees_term_name($feepaydata->fees_id));
