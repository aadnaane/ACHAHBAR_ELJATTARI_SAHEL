...
class StudentListing
{
	...
	public function redirectMethod()
	{
		...
		{		
			...
			$role = mj_smgt_get_role($_REQUEST['student_id']);
...
public function student_listing($data)
{
	...
	if($data["current_user"]!=0)
	{
		...
		$menu_access_data=mj_smgt_get_userrole_wise_access_right_array_in_api($data['current_user'],'student');
		$user_role= mj_smgt_get_role($data["current_user"]);
		$role=$user_role[0];
		//var_dump($menu_access_data['view']);
	...
	{		
		...
		{
			$userimagedata=mj_smgt_get_user_image($retrieved_data->ID);
			...
			if(get_user_meta($retrieved_data->ID, 'roll_id', true))
				...
				if($section_name!='')
				{
					...
					$section=mj_smgt_get_section_name($section_name); 
