...
class ClassRoutine
{
...

	{
...

		{
			...
			$role = mj_smgt_get_roles($userid);

			if ($role == 'teacher')

			{

				$retrieve_class = mj_smgt_get_allclass($userid);
		...
		if ($role == 'student')
		{
			...
			$class = $school_obj->mj_smgt_get_user_class_id($userid);
			$section = get_user_meta($userid, 'class_section', true);
			if (!empty($class))
			{
				if ($section != "")
				{
					$section_name = mj_smgt_get_section_name($section);
				}
				 else
				{
					$section = 0;
...

		{
			...
			if (!empty($retrieve_class))
				...
				foreach($retrieve_class as $class)
					...
					$classsections = mj_smgt_get_class_sections($class['class_id'], $userid);
					...
					foreach($classsections as $section)
						...
						if (get_option('smgt_students_access') == 'own' && $school_obj->role == 'teacher')
							...
							$section = mj_smgt_get_section_name($section);
