	...
	public function virtualclass_list($user_id)
	{
        ...
        if(!empty($meeting_list_data))
        {
            ...
            foreach ($meeting_list_data as $retrieved_data)
            {
                ...
                $subject= mj_smgt_get_single_subject_name($subid);

                $cid=$retrieved_data->class_id;
                $class_name=mj_smgt_get_class_name($cid);
                ...
                $teacher=mj_smgt_get_teacher($retrieved_data->teacher_id);

                $start_date=mj_smgt_getdate_in_input_box($retrieved_data->start_date);

                $end_date=mj_smgt_getdate_in_input_box($retrieved_data->end_date);

                $result[$i]['subject']=$subject;
                $result[$i]['class_name']=$class_name;
                $result[$i]['class_section']=$section_name;
                ...
                $result[$i]['topic']=$retrieved_data->agenda;
                $result[$i]['join_meeting_link']=$retrieved_data->meeting_join_link;
                $i++;
                $response['status']=1;
