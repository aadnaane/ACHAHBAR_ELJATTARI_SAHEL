...
class HomeworkView{
	...
	public function view_homework($homework_id)
	{
		...
		if(!empty($homework)){
						...
						$classname=mj_smgt_get_class_name($homework['class_id']);
						$subjectname=mj_smgt_get_single_subject_name($homework['subject_id']);
						
						if(isset($homework['section_id']) && $homework['section_id']!=0){
								$section=mj_smgt_get_section_name($homework['section_id']); 
						}
						else
						{
