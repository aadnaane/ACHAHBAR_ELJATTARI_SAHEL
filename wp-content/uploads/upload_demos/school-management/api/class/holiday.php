...
class HolidayClass{
	...
	{	
		...
		$holidaydata= mj_smgt_get_all_data($tablename);		
		...
		if(!empty($holidaydata)){
			...
			foreach ($holidaydata as $retrieved_data){ 
				...
				$result[$i]['start_date']=mj_smgt_getdate_in_input_box($retrieved_data->date);
				...
				$result[$i]['end_date']=mj_smgt_getdate_in_input_box($retrieved_data->end_date);
				...
				$result[$i]['status']=mj_smgt_cheak_type_status(get_current_user_id(),'holiday',$retrieved_data->holiday_id);
				$i++;
			}
