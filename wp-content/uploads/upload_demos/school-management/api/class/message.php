...
class MessageClass
{
...

		{
...

			{
...

				{
					...
					$userdata=mj_smgt_get_user_notice($role,$data['class_id'],$data['class_section_id']);
...

				{
					...
					$userdata=mj_smgt_get_user_notice($role,$data['class_id']);
	...
	{	
...

		{
			...
			$messagedata=mj_smgt_get_send_message($data['current_user']);
			...
			{ 
...

				{
...

					{
						...
						$message_for=mj_smgt_get_display_name(get_post_meta( $msg_post->ID, 'message_smgt_user_id',true));
...

					{
						...
						$class=mj_smgt_get_class_name(get_post_meta( $msg_post->ID, 'smgt_class_id',true)); 
	...
	{      
		$menu_access_data=mj_smgt_get_userrole_wise_access_right_array_in_api($data['current_user'],'message');
			...
			{ 
				...
				{ 
...

					{
						...
						$message_for=mj_smgt_get_display_name($msg->sender);
                        ...
                        $message_date= mj_smgt_getdate_in_input_box($msg->date);
...

				{
					...
					$result[$i]['Status']=mj_smgt_cheak_type_status($data['current_user'],'message',$msg->message_id);
					...
					if(!empty($attchment))
					{
						...
						{
								$result[$i]['Attchment'] =  content_url().'/uploads/school_assets/'.$attchment_data;					
						}
					}
					else {
	...
	{		
...

		{
			...
			{	
				...
				$sender=mj_smgt_get_display_name($message->post_author);
...

				{
					...
					$receiver=mj_smgt_get_display_name(get_post_meta($data['message_id'],'message_smgt_user_id',true));
