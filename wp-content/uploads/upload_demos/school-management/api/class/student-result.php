...
class StudentResult{
	...
	public function redirectMethod()
	{
		...
		{
			$response=$this->smgt_api_translate_result_pdf_function($_REQUEST);	 
			if(is_array($response)){
				echo json_encode($response);
			}
			else
...
{	
	...
	{			
		...
		$examdata = mj_smgt_get_all_data('exam');
		$result_status=0;	
		...
		{	
			...
			if($exam->exam_id!=0 && $exam->exam_id!=""){		
				...
				$exam=mj_smgt_get_exam_by_id($exam_id);			
				...
				{					
					...
					foreach($subject as $sub)
					{
						...
						$grade_comment=$obj_mark->mj_smgt_get_grade_marks_comment($grade);
	...
	public function smgt_api_translate_result_pdf_function($data)
	{
		...
		{
			$result=mj_smgt_api_translate_result_pdf($student_id,$exam_id);
			if($result)
			{
