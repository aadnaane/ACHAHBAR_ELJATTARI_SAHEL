...
class SubjectListing
{
    ...
    public function subject_listing($data)
    {
		...
		$role=$user_role[0];
		$menu_access_data=mj_smgt_get_userrole_wise_access_right_array_in_api($data['current_user'],'subject');
		if($role=='student')
		{			
...

			{
				...
				$subjectdata = mj_smgt_get_all_data('subject');
        ...
        if (!empty($subjectdata)) 
		{
...

			{
               ...
               // $uid                        = $retrieved_data->teacher_id;
                ...
                $result[$i]['class']        = mj_smgt_get_class_name($cid);
