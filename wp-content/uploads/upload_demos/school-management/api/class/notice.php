...
{	
	$role = mj_smgt_get_role($data['current_user']);
	$menu_access_data=mj_smgt_get_userrole_wise_access_right_array_in_api($data['current_user'],'notice');	
 	if($role[0]=='student')
	{
		if($menu_access_data['view'] == '1' && $menu_access_data['own_data'] == 1 )
		{
			...
			$noticedata = mj_smgt_student_notice_dashbord($class_name,$class_section);
		}
		...
		{	
			...
			foreach ($noticedata as $retrieved_data)
			{
				...
				{
					$class_name=mj_smgt_get_class_name(get_post_meta( $retrieved_data->ID, 'smgt_class_id',true));
				} 		
				...
				$result[$i]['start_date']	=	mj_smgt_getdate_in_input_box(get_post_meta($retrieved_data->ID,'start_date',true));
				...
				$result[$i]['end_date']	   =	mj_smgt_getdate_in_input_box(get_post_meta($retrieved_data->ID,'end_date',true));
				...
				$result[$i]['Status']		=	mj_smgt_cheak_type_status($data["current_user"],'notice',$retrieved_data->ID);
				$i++;
			}
	...
	{	
		...
		if(!empty($retrieved_data)){
				...
				$result['start_date']=mj_smgt_getdate_in_input_box(get_post_meta($retrieved_data->ID,'start_date',true));
				$result['end_date']=mj_smgt_getdate_in_input_box(get_post_meta($retrieved_data->ID,'end_date',true));
