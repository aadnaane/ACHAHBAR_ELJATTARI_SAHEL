...
class PaymentClass{
	...
	{			
		...
		$menu_access_data=mj_smgt_get_userrole_wise_access_right_array_in_api($data['current_user'],'feepayment');
		$class_id = get_user_meta($_REQUEST['current_user'],'class_name',true);	
		
		if($role=='student'){			
			if($menu_access_data['view'] == '1' && $menu_access_data['own_data'] == 1 )
			{
				$paymentdata	= get_student_mj_smgt_payment_list($data["current_user"]);
			}
		...
		if(!empty($paymentdata))
		{
			...
			foreach ($paymentdata as $retrieved_data){ 
				...
				$result[$i]['student']=mj_smgt_get_user_name_byid($retrieved_data->student_id);
				$result[$i]['roll_no']=get_user_meta($retrieved_data->student_id, 'roll_id',true);
				$result[$i]['class']=mj_smgt_get_class_name($retrieved_data->class_id);
				$result[$i]['payment_title']=$retrieved_data->payment_title;
				$result[$i]['amount']=$retrieved_data->amount;
				$result[$i]['status']=$status;
				$result[$i]['date']= mj_smgt_getdate_in_input_box($retrieved_data->date);
				$i++;
			}
	...
	{	
		$retrieved_data=mj_smgt_get_payment_by_id($data['payment_id']);
		if(!empty($retrieved_data)){
			$i=0;
				...
				$result['student']=mj_smgt_get_user_name_byid($retrieved_data->student_id);
				$result['roll_no']=get_user_meta($retrieved_data->student_id, 'roll_id',true);
				$result['class']=mj_smgt_get_class_name($retrieved_data->class_id);
				$result['payment_title']=$retrieved_data->payment_title;
				$result['amount']=$retrieved_data->amount;
				$result['status']=$status;
				$result['date']=mj_smgt_getdate_in_input_box($retrieved_data->date);
	...
	{	
		...
		if(!empty($incomedata)){
			...
			foreach ($incomedata as $retrieved_data){ 
				...
				$result[$i]['student']=mj_smgt_get_user_name_byid($retrieved_data->supplier_name);
				$result[$i]['amount']=$total_amount;
				$result[$i]['date']=mj_smgt_getdate_in_input_box($retrieved_data->income_create_date);
				$i++;
			}
			$response['status']=1;						$response['date_formate']	= get_option('smgt_datepicker_format');
			$response['resource']=$result;
	...
	{	
		...
		if(!empty($retrieved_data)){
				...
				$result['student']=mj_smgt_get_user_name_byid($retrieved_data->supplier_name);
				$result['amount']=$total_amount;
				$result['date']=mj_smgt_getdate_in_input_box($retrieved_data->income_create_date);
