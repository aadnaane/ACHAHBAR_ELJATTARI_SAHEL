...
<h2 class="nav-tab-wrapper">
	...
	<?php echo '<span class="dashicons dashicons-edit"></span>'. esc_attr__('Manage Marks', 'school-mgt'); ?></a>
	<a href="?page=smgt_result&tab=export_marks" class="nav-tab <?php echo $active_tab == 'export_marks' ? 'nav-tab-active' : ''; ?>">
	<?php echo '<span class="dashicons dashicons-upload"></span>'. esc_attr__('Export Marks', 'school-mgt'); ?></a>		
	...
	<?php echo '<span class="dashicons dashicons-editor-alignleft"></span>'. esc_attr__('Add Multiple Subject Marks', 'school-mgt'); ?></a>
</h2>
<?php
$tablename="marks";
...
$(document).ready(function() {
		...
		<form method="post" id="select_data">  
			<div class="row">
				...
				<div class="form-group col-md-3 button-possition">
					...
					<input type="submit" value="<?php esc_attr_e('Manage Marks','school-mgt');?>" name="manage_mark"  class="btn btn-info"/>
				</div>
			</div>
    ...
    <label for="file"><?php esc_attr_e('Select CSV file','school-mgt')?>:  </label>   <input type="file" name="csv_file" id="csv_file" class="d-inline" />
      <input type="submit" name="upload_csv_file" value="<?php esc_attr_e('Fill data from CSV File','school-mgt');?>" class="btn btn-info" /> 
	<?php esc_attr_e('CSV file Must have headers as follows','school-mgt');?>: <?php esc_attr_e('roll_no, name, marks, comment','school-mgt');?>
      <br /><p></p>
	<div class="table-responsive">
		...
		<?php
			...
			foreach ( $student as $user ) {
				...
				echo '<tr>';
				echo '<td><span '.(isset($csv) && !(isset($key))? 'class="bg-danger">': '>' ).$user->roll_id. '</span></td>';
				echo '<td><span>' .$user->first_name.' '.$user->last_name. '</span></td>';
				echo '<td id="position_relative">';
				echo '<input type="text" name="marks_'.$user->ID.'" value="'.(isset($key)? $csv[$key]['marks'] : $marks).'" class="form-control validate[required] text-input">';
				echo '</td>';
				...
				echo '<button type="submit" name="add_mark" value="'.$user->ID.'" class="btn btn-success p-2">'.$button_text.'</button>';			
				echo '</td>';
				echo '</tr>';
