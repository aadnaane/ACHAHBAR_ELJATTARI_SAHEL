	...
	<div class="table-responsive">
		<table class="table">
			<thead>
				<tr>
				   ...
				   $prev = $p - 1;
					$next = $p + 1;
					$limits = (int)($p - 1) * $max;
					$totlal_message = mj_smgt_count_send_item(get_current_user_id());
					$totlal_message = ceil($totlal_message / $max);
					$lpm1 = $totlal_message - 1;               	
					$offest_value = ($p-1) * $max;
					echo mj_smgt_pagination($totlal_message,$p,$lpm1,$prev,$next);
			...
			<tr> 			
				...
				<th>
					<span><?php esc_attr_e('Class','school-mgt');?></span>
				</th>
			...
			foreach($message as $msg_post)
			{
				...
				?>
				<tr>
				<td><a href="?page=smgt_message&tab=view_message&from=sendbox&id=<?php echo $msg_post->ID;?>" class="text_decoration_none">
					<span>
					<?php 
				...
				<td class="width_100px"><a href="?page=smgt_message&tab=view_message&from=sendbox&id=<?php echo $msg_post->ID;?>" class="text_decoration_none">
				...
				?>
				<?php if(mj_smgt_count_reply_item($msg_post->ID)>=1){?><span class="badge badge-success pull-right"><?php echo mj_smgt_count_reply_item($msg_post->ID);?></span><?php } ?></a>
				</td>
				<td class="width_400px"><a href="?page=smgt_message&tab=view_message&from=sendbox&id=<?php echo $msg_post->ID;?>" class="text_decoration_none">
				<?php
