<?php
class VIBE_Options_radio_img extends VIBE_Options{	
	...
	function render(){
		...
		echo '<fieldset>';
			...
			foreach($this->field['options'] as $k => $v){
				...
				echo '<img src="'.$v['img'].'" alt="'.$v['title'].'" onclick="jQuery:vibe_radio_img_select(\''.$this->field['id'].'_'.array_search($k,array_keys($this->field['options'])).'\', \''.$this->field['id'].'\');" />';
				echo '<br/><span>'.empty($v['title'])?'':$v['title'].'</span>';
				echo '</label>';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			'vibe-opts-field-radio_img-js', 
			VIBE_OPTIONS_URL.'fields/radio_img/field_radio_img.js', 
			array('jquery'),
			time(),
			true
		);
